...
export function CheckinOverview({
  ...
  const emoRecs = todayCheckin
    ? getEmotionRecommendations(emotion.key)
        .filter((r) => (r.cadence ?? "daily") === "daily")
        .slice(0, maxRecs)
    : [];
  ...
  return (
    <section id="health-overview" className={cn(view === "all" && "mt-8 scroll-mt-24")}>
      ...
      {/* 오늘 권고 — view='today' 일 때는 1개만 강조, 'all'일 때는 maxRecs개 */}
      {emoRecs.length > 0 && (
        <section className={cn(view === "today" ? "mt-6" : "mt-10")}>
          {view !== "today" && (
            <h3 className="font-display text-fluid-3xl text-foreground">오늘 하면 좋은 것</h3>
          )}
          <p className={cn("text-fluid-base text-foreground/65", view === "today" ? "mt-0" : "mt-3")}>
            목소리에서 <span className={cn("font-semibold", emotion.textTone)}>{emotion.label}</span> 신호가 느껴졌어요.
          </p>
          <ul className={cn("space-y-4", view === "today" ? "mt-4" : "mt-6")}>
            {emoRecs.map((r, i) => (
              <li key={i} className="flex gap-4 rounded-3xl border-2 border-border/70 bg-surface-elevated p-5">
                <span className={cn("shrink-0 self-start rounded-full border px-3 py-1 text-fluid-xs font-bold", toneByPriority[r.priority] ?? toneByPriority.keep)}>
                  {REC_PRIORITY_LABEL[r.priority]}
                </span>
                <div className="min-w-0">
                  <p className="text-fluid-xl font-semibold leading-snug text-foreground">{r.text}</p>
                  {r.hint && <p className="mt-2 text-fluid-base leading-relaxed text-foreground/65">{r.hint}</p>}
                </div>
              </li>
            ))}
          </ul>
          <p className="mt-4 text-fluid-sm text-foreground/55 text-xs text-center font-sans">
            ※ 의료 진단이 아니에요. 증상이 반복되면 보호자나 의료기관과 상담해주세요.
          </p>
        </section>
      )}
      </>
      ...
      {showWeek && (
        <section className={cn(view === "week" ? "" : "mt-10")}>
          ...
          {/* 이번 주 감정 기반 권고 (논문 근거) */}
          {view === "week" && recsByCadence && recsByCadence.weekly.length > 0 && (
            <CadenceRecBlock
              title="이번 주 해보면 좋은 것"
              caption={`${emotion.label} 신호 기반`}
              items={recsByCadence.weekly}
        ...
        <section className={cn(view === "details" ? "" : "mt-10")}>
          {view === "all" ? (
            ...
            <>
              ...
              {/* 자세히 탭 — 주간 + 월간 권고 모음 */}
              {recsByCadence && (recsByCadence.weekly.length > 0 || recsByCadence.monthly.length > 0) && (
                <div className="mt-10">
                  <h3 className="font-display text-fluid-2xl text-foreground">
                    감정 기반 권고 모음
                  </h3>
                  <p className="mt-2 text-fluid-base text-foreground/65">
                    오늘 분석된 <span className={cn("font-semibold", emotion.textTone)}>{emotion.label}</span> 신호에 맞춘 단·중·장기 권고예요.
                  </p>
                  {recsByCadence.weekly.length > 0 && (
                    <CadenceRecBlock
                      title="이번 주"
                      items={recsByCadence.weekly}
                      toneByPriority={toneByPriority}
                  ...
                  {recsByCadence.monthly.length > 0 && (
                    <CadenceRecBlock
                      title="이번 달"
                      items={recsByCadence.monthly}
                      toneByPriority={toneByPriority}
                    />
                  )}
                  <p className="mt-4 text-fluid-sm text-foreground/55 text-xs text-center font-sans">
    ...
    <>
      <div className="mt-8">
        ...
        <ul className="mt-5 grid grid-cols-1 gap-x-6 gap-y-1 sm:grid-cols-2">
          {axes.map((a) => (
            <li key={a.key} className="flex items-center justify-between border-b border-border/60 py-4">
              <div className="flex items-center gap-3">
                <span className={cn("h-3 w-3 rounded-full", STATUS_DOT[a.status])} />
                <span className="text-fluid-lg text-foreground">{a.label}</span>
              </div>
              <span className="text-fluid-base text-foreground/60">{STATUS_LABEL[a.status]}</span>
            </li>
          ))}
        </ul>
      </div>
        ...
        <div className="mt-5 grid grid-cols-10 gap-2">
          {calendar.map((d) => (
            <div key={d.date} className="flex flex-col items-center gap-1.5">
              <span className={cn("h-4 w-4 rounded-full", LEVEL_DOT[d.level])} title={`${d.label} · ${d.level}`} />
              <span className="text-[11px] tabular-nums text-foreground/55">{d.label}</span>
            </div>
          ))}
        </div>
      </div>
...
function CadenceRecBlock({
  ...
  title: string;
  caption?: string;
  items: { priority: "now" | "soon" | "keep"; text: string; hint?: string; evidence?: string }[];
    ...
    <div className="mt-8">
      ...
      <ul className="mt-4 space-y-3">
        {items.map((r, i) => (
          <li
            key={i}
            className="flex gap-3 rounded-2xl border-2 border-border/60 bg-surface-elevated p-4"
          >
            <span
              className={cn(
                "shrink-0 self-start rounded-full border px-2.5 py-1 text-fluid-xs font-bold",
            ...
            <div className="min-w-0">
              <p className="text-fluid-lg font-semibold leading-snug text-foreground">{r.text}</p>
              {r.hint && (
                <p className="mt-1.5 text-fluid-base leading-relaxed text-foreground/65">
                  {r.hint}
                </p>
              )}
              {r.evidence && (
                <p className="mt-2 text-fluid-sm italic text-foreground/50">근거: {r.evidence}</p>
              )}
            </div>
          </li>
        ))}
